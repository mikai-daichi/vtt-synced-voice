...
- マージ後の長すぎるキューを後処理で分割（`min_cue_chars`、デフォルト50文字）：句点があれば全句点で分割、なければ形態素解析で文末+文頭パターンを検出して分割
...
transcribe(
    ...
    min_cue_chars=50,         # マージ後にこの文字数を超えるキューを後処理分割する（0で無効）
...
マージ後にこの文字数を超えるキューを後処理で分割します（デフォルト: `50`）。

**フェーズ1（句点による分割）**：テキスト内に `。` `！` `？` が含まれる場合、全ての句点位置で分割します。

**フェーズ2（形態素解析による分割）**：句点がない場合は Janome で形態素解析を行い、「文末トークン（です・ます・た・よ 等）」の直後に「文頭らしいトークン（名詞・代名詞・感動詞・接続詞 等）」が来るパターンで分割します。

`50` 文字以下のキューは一切変更されません。`0` を指定すると後処理分割を無効にできます。
